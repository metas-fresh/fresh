...
FROM M_ProductPrice pp
    ...
    ) bpProductPackingMaterial ON (bpProductPackingMaterial.M_HU_PI_Item_Product_ID = pp.m_hu_pi_item_product_id or (bpProductPackingMaterial.M_HU_PI_Item_Product_ID is null and pp.m_hu_pi_item_product_id is null))
    ...
    WHERE pp2.M_Product_ID = pp.M_Product_ID
      AND (pp.m_hu_pi_item_product_id = bpProductPackingMaterial.m_hu_pi_item_product_id OR (pp.m_hu_pi_item_product_id IS NULL AND bpProductPackingMaterial.m_hu_pi_item_product_id IS NULL))
      ...
      AND (ppa.signature = ppa2.signature)
         ...
         LEFT OUTER JOIN m_hu_pi_item_product hupip ON (CASE
                                                            WHEN pp.m_hu_pi_item_product_id IS NULL
                                                                ...
                                                                ELSE hupip.m_product_id = bpProductPackingMaterial.m_product_id and pp.m_hu_pi_item_product_id = hupip.m_hu_pi_item_product_id AND hupip.isActive = 'Y'
                                                        END)
